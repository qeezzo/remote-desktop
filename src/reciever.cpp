...
int main()
{
    ...
    try {
        ...
        //////ScreenData scrData;
        //////Packet* pack = encoder.encodeFrame(scrData.screen_data(width, height));

        //Packet* pack{};
        //while (true) {
        //    pack = recievePack(&client);
        //    FrameRGBA* frame{};

        //    if (pack) {
        //        cout << "Decoding packet..." << endl;
        //        frame = decoder.decodePacket(pack);
        //    } else break;

        //}

        //if (frame) ximage(frame);
        ximage(nullptr);
...
int ximage(FrameRGBA* frame) {
    ...
    //image = XCreateImage(display, DefaultVisual(display, screen), DefaultDepth(display, screen), ZPixmap, 0, (char*)frame->data(), width, height, 32, 0);
